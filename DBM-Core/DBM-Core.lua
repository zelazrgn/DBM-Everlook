...
local DBMPrefix = "D4E" -- isRetail and "D4" or isClassic and "D4C" or isBCC and "D4BC"
...
DBM = {
	Revision = parseCurseDate("20231006153943"),
}
-- The string that is shown as version
if isClassic then
	DBM.DisplayVersion = "1.14.0"
	DBM.ReleaseRevision = releaseDate(2023, 3, 31) -- the date of the latest stable version that is available, optionally pass hours, minutes, and seconds for multiple releases in one day
end
...
do
	...
	local nextCombatCheck = 0
	--mainFrame:SetScript("OnUpdate", function(self, elapsed)
	local function onUpdate(self, elapsed)
		local time = GetTime()

		if time > nextCombatCheck and InCombatLockdown() then
			nextCombatCheck = time + 0.05
			DBM:checkForCombat()
		end
...
do
	...
	syncHandlers["C"] = function(sender, delay, mod, modRevision, startHp, dbmRevision, modHFRevision, event)
		...
		if not cSyncSender[sender] then
			...
			if cSyncReceived > 0 then -- need at least 3 sync to combat start. (for security)
	...
	function DBM:CHAT_MSG_ADDON(prefix, msg, channel, sender)
		...
		elseif prefix == "BigWigs" and msg and (channel == "PARTY" or channel == "RAID" or channel == "INSTANCE_CHAT") then
			...
			local bwOldPrefix, bwOldMsg, bwOldExtra = strsplit(" ", msg)
			...
			elseif bwOldPrefix and bwOldMsg then
				if bwOldPrefix == "BossEngaged" then--Boss Mod Sync Old
					-- sender, delay, mod, modRevision, startHp, dbmRevision, modHFRevision, event
					handleSync(channel, sender, "C", 0, bwOldMsg, -1, -1, 20211006153943)
				end
			end
...
do
	...
	local function checkForPull(mob, combatInfo)
		...
		scanForCombat(combatInfo.mod, mob, 0.0001)
		-- DBM:Schedule(0.1, scanForCombat, combatInfo.mod, mob, 0.1)
	...
	function DBM:checkForCombat()
		if not combatInitialized then return end
		if not dbmIsEnabled then return end
		if not combatInfo[LastInstanceMapID] then return end
		for _, v in ipairs(combatInfo[LastInstanceMapID]) do
			if v.mod and v.mod.inCombat then
				return
			end
		end
		for _, v in ipairs(combatInfo[LastInstanceMapID]) do
...
do
	...
	-- newTimer(self, "cd", 15, 19715, nil, "SpellCaster", nil, 3)--15-19
